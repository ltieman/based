...
def test_archive(first_post):
    item = UnitTestBaseCrud.delete(session=session, id=first_post.id)
...
def test_unarchived(first_post):
    ...
    assert item.archived
    assert item.id == first_post.id
    unarchived = UnitTestBaseCrud.undelete(session=session, id=first_post.id)
...
def test_head():
    head = UnitTestBaseCrud.head(
        session=session, params=UnitTestBaseCrud.query_params()
    )
    assert int(head) == index_count


def test_index():
    index = UnitTestBaseCrud.index(
        session=session, params=UnitTestBaseCrud.query_params()
    )
    assert len(index) == 20
    index = UnitTestBaseCrud.index(
        session=session, params=UnitTestBaseCrud.query_params(offset=20)
    )
    assert len(index) == 20
    index = UnitTestBaseCrud.index(
        session=session, params=UnitTestBaseCrud.query_params(offset=40)
    )
    assert not index
