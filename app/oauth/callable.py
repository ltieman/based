...
class AuthCallable:
    ...
    def validate_code(self, session: Session, code: str):
        try:
            validated_user = AuthCrud.validate_code(session=session, code=code)
            validated_user.authorized_groups = [
                group_role.group_id
...
class AuthRoleCheck(AuthCallable):
    ...
    def auth_logic(self, user: UserWithRoles):
        ...
        if self.role:
            roles = [
                role for role in user.roles if RoleEnum[role].value >= self.role.value
            ]
            if roles:
                auth_status = True
        if (
            hasattr(self.model, "group_id") or self.model.__name__.lower() == "group"
        ) and user.authorized_groups:
            auth_status = True
        if auth_status:
            return user
        else:
...
def AuthRoleOrSelfCheck(
    self, role: List[str], required: bool = False, model: BaseModel = None
):
